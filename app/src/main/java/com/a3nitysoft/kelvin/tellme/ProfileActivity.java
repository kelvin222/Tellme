...
public class ProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mProList = (RecyclerView) findViewById(R.id.profile_list_view);

        FirebaseAuth firebaseAuth = FirebaseAuth.getInstance();

        accountAdapter = new AccountAdapter(blog_list);
        mProList.setLayoutManager(new LinearLayoutManager(this));
        mProList.setAdapter(accountAdapter);
        mProList.setHasFixedSize(true);
                ...
                if (task.isSuccessful()) {
                    if (task.getResult().exists()) {
                        ...
                        if (!Pimage.equals("default")) {
                            Picasso.with(ProfileActivity.this).load(Pimage).placeholder(R.drawable.dp).into(mProfileImage);
                                                    ...
                                                    public void onComplete(@NonNull Task<Void> task) {
                                                        if (task.isSuccessful()) {
                                                            ...
                                                            firebaseFirestore.collection("Users/" + mCuser + "/tagged").document(user_id).set(audi);
                                                                                    ...
                                                                                    Toast.makeText(ProfileActivity.this, "Unable to Tag this user, Try later. ", Toast.LENGTH_SHORT).show();
